    ...
    <div className="flex items-center gap-1 overflow-x-auto pb-2">
      {STEPS.map((label, i) => (
        <div key={i} className="flex items-center">
          ...
          {i < STEPS.length - 1 && (
            <div
              className={`w-6 h-0.5 shrink-0 ${i < current ? 'bg-green-600' : 'bg-gray-200'}`}
            />
          )}
        </div>
      ))}
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <Input value={data.firstName} onChange={(e) => update('firstName', e.target.value)} placeholder="Jane" />
          <FieldError msg={errors.firstName} />
        </div>
        <div>
          <Label required>Last Name</Label>
          <Input value={data.lastName} onChange={(e) => update('lastName', e.target.value)} placeholder="Smith" />
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <Input value={data.unit} onChange={(e) => update('unit', e.target.value)} placeholder="Apt 4B" />
        </div>
        <div>
          <Label required>Postal Code</Label>
          <Input value={data.postalCode} onChange={(e) => update('postalCode', e.target.value.toUpperCase())} placeholder="A1B 2C3" />
          <FieldError msg={errors.postalCode} />
        </div>
      </div>
      <div>
        <Label>Buzzer / Gate Code or Note</Label>
        <Input value={data.buzzerNote} onChange={(e) => update('buzzerNote', e.target.value)} placeholder="Buzzer #204, gate code 1234" />
      </div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-gray-500">
        Upload {min}–{max} photos of the items you need removed. Clear photos help us give you an accurate quote faster.
      </p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {photos.map((file, i) => (
            <div key={i} className="relative group">
              <img src={URL.createObjectURL(file)} alt={`photo-${i + 1}`} className="w-full h-24 object-cover rounded-lg border border-gray-200" />
              <button
                type="button"
                onClick={() => remove(i)}
      ...
      {photos.length < max && (
        ...
        >
          ...
          <span className="text-sm font-medium">Click to add photos ({photos.length}/{max})</span>
          <span className="block text-xs text-gray-400 mt-1">JPG, PNG, HEIC — up to {max} photos</span>
        </button>
      )}
...
function Step4({ data, update, errors }) {
  ...
  return (
    <div className="space-y-5">
      <h3 className="text-xl font-bold text-gray-900">Junk Details</h3>
      ...
      <div>
        ...
        <Textarea
          ...
          placeholder="Describe what needs to go — e.g. old couch, broken TV stand, boxes of clothes..."
        />
          ...
          {[
            ...
            <button
              ...
              className={`border rounded-lg p-3 text-left transition-colors ${
                data.jobType === value ? 'border-green-600 bg-green-50' : 'border-gray-200 hover:border-green-300'
              }`}
            >
...
function Step5({ data, update, errors }) {
  ...
  return (
    <div className="space-y-5">
      <h3 className="text-xl font-bold text-gray-900">Access & Preferred Time</h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <Input type="date" min={minDate} value={data.preferredDate} onChange={(e) => update('preferredDate', e.target.value)} />
...
function Step6({ data, photos }) {
  const sections = [
    ...
    },
    {
      ...
      rows: [
        ...
        data.itemCategories.length ? ['Categories', data.itemCategories.join(', ')] : null,
        data.jobType ? ['Job Size', data.jobType] : null,
        data.specialItems.length ? ['Special Items', data.specialItems.join(', ')] : null,
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-gray-500">Please review your details before submitting. Use the Back button to make changes.</p>
      ...
      {photos.length > 0 && (
        <div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
            {photos.map((file, i) => (
              <img key={i} src={URL.createObjectURL(file)} alt={`preview-${i + 1}`} className="w-full h-20 object-cover rounded-lg border border-gray-200" />
            ))}
          </div>
        </div>
      )}
...
function validateStep(step, data, photos, photoLimits) {
  ...
  if (step === 2) {
    if (photos.length < photoLimits.min) errors.photos = `Please upload at least ${photoLimits.min} photos.`
  }
  if (step === 3) {
    if (!data.whatNeedsRemoval.trim()) errors.whatNeedsRemoval = 'Please describe what needs to be removed.'
  }
  if (step === 4) {
    if (!data.preferredDate) errors.preferredDate = 'Please choose a preferred date.'
    if (!data.preferredTimeWindow) errors.preferredTimeWindow = 'Please choose a time window.'
  }
  return errors
}
...
export default function BookingForm() {
  ...
  // countdown + auto-reset after successful submission
  useEffect(() => {
    if (!submitted) return
    setCountdown(30)
    ...
    <section id="book" className="py-20 px-4 bg-white">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-10">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 mt-2">Book a Pickup</h2>
          <p className="text-gray-500 mt-3">Fill out the form below and we'll get back to you with a quote.</p>
        </div>
        ...
        {submitted ? (
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 text-center">
            ...
            <ol className="text-left space-y-3 mb-8 max-w-sm mx-auto">
              {[
                ...
                <li key={i} className="flex items-start gap-3 text-sm text-gray-600">
                  <span className="w-6 h-6 bg-green-600 text-white rounded-full flex items-center justify-center shrink-0 text-xs font-bold">
                    {i + 1}
                  </span>
                  {text}
                </li>
            ...
            <p className="text-xs text-gray-400">
              This form will reset in <span className="font-semibold text-gray-600">{countdown}s</span>
            </p>
          </div>
        ) : (
          <>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sm:p-8">
              ...
              {step === 2 && <Step3 photos={photos} setPhotos={setPhotos} errors={errors} photoLimits={photoLimits} />}
